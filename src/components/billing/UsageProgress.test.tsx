...
describe('UsageProgress', () => {
  ...
  it('shows counts and Normal badge under 75%', async () => {
    ...
    renderWithProviders(
      <UsageProgress metric="messages" title="Messages" description="This month" />,
    )
  ...
  it('shows Warning badge between 75% and 90%', async () => {
    ...
    renderWithProviders(
      <UsageProgress metric="files" title="Files" />,
    )
  ...
  it('shows Critical badge at 90%+ and limit exceeded notice when blocked', async () => {
    ...
    renderWithProviders(
      <UsageProgress metric="agents" title="Agents" />,
    )

    expect(await screen.findByText('Critical')).toBeInTheDocument()
    expect(screen.getByText('Limit exceeded. Upgrade your plan to continue.')).toBeInTheDocument()
  })
})
