					...
					>
						<div className="flex items-center gap-2">
							{config.branding.showHeaderIcon && (
								<div
									className={`w-8 h-8 bg-white/20 flex items-center justify-center ${
										config.branding.headerIconCircular
											? "rounded-full"
											: "rounded"
									}`}
								>
					...
					<div className="flex-1 p-4 overflow-y-auto min-h-0">
						...
						{texts.greetingMessages.map((msg, i) => (
							<div key={i} className="flex gap-2 mb-3">
								<div
									className={`w-6 h-6 bg-gray-200 flex items-center justify-center flex-shrink-0 ${
										config.branding.botAvatarCircular
											? "rounded-full"
											: "rounded"
									}`}
								>
