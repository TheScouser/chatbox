...
function TextExtractionDemo() {
  ...
  const files = useQuery(api.files.getFilesForAgent, firstAgent ? { agentId: firstAgent._id } : "skip")
  const knowledgeEntries = useQuery(api.knowledge.getKnowledgeForAgent, firstAgent ? { agentId: firstAgent._id } : "skip")
  ...
  const handleFileUploadComplete = async (fileId: string) => {
    try {
      ...
      setMessage({ 
        type: 'error', 
        text: error instanceof Error ? error.message : 'Failed to process uploaded file' 
      })
    } finally {
      ...
      <div className="min-h-screen bg-gray-50 py-8">
        <div className="max-w-6xl mx-auto px-4">
          <div className="bg-white rounded-lg shadow p-6">
            <h1 className="text-2xl font-bold text-gray-900 mb-6">Text Extraction Demo</h1>
              ...
              <div className="space-y-8">
                <div>
                  <p className="text-gray-600 mb-6">
                    Testing text extraction with agent: <strong>{firstAgent.name}</strong>
                  </p>
                          ...
                          <div key={entry._id} className="p-4 bg-gray-50 rounded-lg">
                            <div className="flex items-start justify-between mb-2">
                              ...
                              <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                                ...
                                {entry.source === 'document' ? 'Document' : 'Text'}
                              </span>
                            </div>
                            <div 
                            ...
                            <div className="flex items-center justify-between mt-2 text-xs text-gray-500">
                              {entry.sourceMetadata?.filename && (
                                <span className="truncate">{entry.sourceMetadata.filename}</span>
                              )}
                              <span>{new Date(entry._creationTime).toLocaleDateString()}</span>
                            </div>
                          </div>
                        ))}
                      </div>
                ...
                <div className="mt-8 pt-8 border-t border-gray-200">
                  <h2 className="text-lg font-medium text-gray-900 mb-4">Phase 5 Status</h2>
                  <div className="space-y-2 text-sm text-gray-600">
                    ...
                    <div className="flex items-center">
                      <span className="text-green-500 mr-2">✓</span>
                      Task 5.3: Text Extraction Action Complete (Basic Implementation)
                    </div>
                    <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                      <p className="text-sm text-yellow-800">
                        <strong>Note:</strong> Text extraction currently supports TXT files fully. 
                        PDF, DOC, and DOCX files show placeholder messages. 
                        To enable full document processing, install libraries like pdf-parse, mammoth, or officeparser.
                      </p>
                    </div>
                  </div>
                </div>
